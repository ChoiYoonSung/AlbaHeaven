	...
	public void getHire() {
		...
		if(0<input&&input<=list.size()){
				for (int i = 0; i < hireKey.length-1; i++) {
					System.out.println(hireKeyname[i]+" : \t\t"+list.get(input-1).get(hireKey[i]));
					}
				System.out.println(hireKeyname[hireKey.length-1]+" : \t\t"+sdf.format(list.get(input-1).get(hireKey[hireKey.length-1])));
				...
				System.out.println("■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■");
					for (int i = 0; i < list2.size(); i++) {
						System.out.println(i+1+" \t"+list2.get(i).get("HR_LIKE")+"라고 관심표현했습니다.");
					}
				System.out.println("■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■");
		}
		else{
